...
const AddSellProduct = () => {
  ...
  const onSubmit = (data) => {
    ...
    const addProduct={name:data.name.toLowerCase(),seller:data.seller.toLowerCase(),price:data.price,sellprice:data.sellprice,shipping:data.shipping,sellshipping:data.sellshipping,category:data.category.toLowerCase(),stock:data.stock,star:rating,starCount:data.starCount,features:inputList}
   ...
   const postData=(data,image)=>{
    ...
    return (
        <Box sx={{mt:5,bgcolor:'whitesmoke',p:2}}>
          ...
          <Typography sx={{textAlign:'start',fontWeight:'bold'}} gutterBottom variant="h5"  component='div'>Add New Product:</Typography>
           ...
           <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
                ...
                />
                {errors.price && "Please enter the product sell price"}
                </div>
                ...
                <input
                    ...
                    className="user-register"
                    style={{marginTop:'6px'}}
                    placeholder='Regular Shipping Cost'
                    type="number"
                    step="0.01"
                    {...register('shipping', {
                    required: true,min:0,
                ...
                <input
                    ...
                    className="user-register"
                    style={{marginTop:'6px'}}
                    placeholder='Sell Shipping Cost'
                    type="number"
                    step="0.01"
                    {...register('sellshipping', {
                    required: true,min:0,

                    })}
                />
                {errors.shipping && "Please enter the sell shipping cost"}
                </div>
